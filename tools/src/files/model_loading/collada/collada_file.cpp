...
namespace undicht {
	...
	namespace tools {
		...
		void ColladaFile::getTexture(TextureData& loadTo_texture, int id) {
		...
		void ColladaFile::loadAllTextures(std::vector<TextureData>& loadTo_textures) {

			// all materials stored in the file
			std::vector<XmlElement*> materials = getAllElements({ "COLLADA", "library_materials", "material" });

			for (XmlElement* material : materials) {
				loadTo_textures.emplace_back(TextureData());
		...
		void ColladaFile::loadMaterialTextures(XmlElement* material, TextureData& loadTo_texture) {
			...
			loadTextureFromFile(loadTo_texture, getFilePath(getFileName()) + image_file_name->getContent());
